...
#     vdotn = np.dot(v.ravel(),n.ravel())
#     left_side = (np.divide((np.dot(v,n.T)),vdotn))
#     right_side = ((np.dot(q.ravel(),n.ravel()))/(np.dot(v.ravel(),n.ravel())))*v
#     xPrime = (np.dot((np.subtract(I, left_side)), x)) + right_side
#     print(xPrime.ravel())
# except Exception:
#     print("invalid computation")
...
def main(lines):
    ...
    for line in lines[0:]:
        p = np.array([[line[0]], [line[1]], [line[2]]])
        q = np.array([[line[3]], [line[4]], [line[5]]])
        r = np.array([[line[6]], [line[7]], [line[8]]])
        try:
